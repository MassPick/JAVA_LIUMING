...
public class InsertBuildManagerFromCBToPG {
	...
	static int i = 0;
	private static Logger log = LogManager.getLogger(InsertBuildManagerFromCBToPG.class);
	static Bucket bucket = null;

	static Connection conn = null;
	static PreparedStatement stmtInsert = null;
	// 19个
	static String sqlInsert = "INSERT INTO build_manager_t(build_base_id,company_name,incident ,incident_link ,incident_time  )"
			+ "VALUES(?::uuid,    ?,?,?,?  )";
			...
			if (null == key) {
				...
				while (true) {
					...
					try {
						result = template.getForObject(
								"http://192.168.1.30:8092/buildManager/_design/dd/_view/dd?inclusive_end={inclusive_ends}&limit={limits}&skip=0",
				...
				while (true) {
					try {
						...
						urlVariables.put("startkeys", "\"" + key + "\"");
						result = template.getForObject(
								"http://192.168.1.30:8092/buildManager/_design/dd/_view/dd?inclusive_end={inclusive_ends}&limit={limits}&startkey={startkeys}&skip=1",
