...
import cn.com.szgao.dto.FfBuildHonourVO;
import cn.com.szgao.dto.FfBuildVO;
import cn.com.szgao.dto.RowsBuildHonourVO;
...
public class InsertBuildHonourFromCBToPG_N {
	...
	static int i = 0;
	private static Logger log = LogManager.getLogger(InsertBuildHonourFromCBToPG_N.class);
	static Bucket bucket = null;

	static Connection conn = null;
	static PreparedStatement stmtInsert = null;
	// 19个
	static String sqlInsert = "INSERT INTO build_honour_t_n(build_base_id,company_name,honor_incident ,incident_link,incident_time  )"
			+ "VALUES(?::uuid,    ?,?,?,?  )";
	...
	public static void doListData3() throws UnsupportedEncodingException {
		...
		while (true) {
			if (null == key) {
				...
				while (true) {
					...
					try {
						result = template.getForObject(
								"http://192.168.1.13:8092/buildHonour/_design/dd/_view/dd?inclusive_end={inclusive_ends}&limit={limits}&skip=0",
				...
				while (true) {
					try {
						...
						urlVariables.put("startkeys", "\"" + key + "\"");
						result = template.getForObject(
								"http://192.168.1.13:8092/buildHonour/_design/dd/_view/dd?inclusive_end={inclusive_ends}&limit={limits}&startkey={startkeys}&skip=1",
			...
			FfBuildHonourVO jsn = gson.fromJson(value, FfBuildHonourVO.class);

			for (RowsBuildHonourVO r : jsn.getRows()) {
				key = r.getKey();
				i++;
				System.out.println(i + ":" + key);
				BuildHonourVO whVo = r.getValue();// 00006a27-0c95-48a3-8249-645457d71b65
				if (whVo == null) {
					continue;
				}
