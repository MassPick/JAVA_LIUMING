...
public class EnterpriseJsonFileIntoDataBase {
	/**
	 * json与对象互转
	 */
	...
	//日志对象
	private static Logger log = LogManager.getLogger(EnterpriseJsonFileIntoDataBase.class);
	//根据字符串生成UUID对象
	static NameBasedGenerator nbg = Generators.nameBasedGenerator(NameBasedGenerator.NAMESPACE_DNS);
	//股东数组
	static JSONArray tuziArray = null;
	//股东详情数组
	static JSONArray chuziArray = null;
	...
	public static void main(String[] args){
		PropertyConfigurator.configure("D:\\xcy\\Maven_Space001\\Mass\\log\\log4j.properties");
		//初始化数据库的行政区数据
		u.initData();
		File dicFile = new File("D:\\xcy\\Data-copy-2015-10-14\\广西---\\广西");//   E://Temp_File//工商数据//pao
		try {
			show(dicFile);
		} catch (IOException e) {
			log.info(e);
	...
	private static void readFileByLines(File file) throws Exception{		
				...
				for(int i = 0; i < tuziArray.size();i++)
				{
					...
					while(chziIter.hasNext())
					{
						...
						if("股东".equals(tuziKey)||"姓名".equals(tuziKey)||"合伙人".equals(tuziKey)
								||"发起人".equals(tuziKey)||"出资人".equals(tuziKey)
								||"投资人".equals(tuziKey)||"投资人名称".equals(tuziKey))
						{
		...
		if(null != holderList && holderList.size()>0)
		{
			for(HolderVO holderVO:holderList)
			{
				...
				Bucket bucket = ClusterUtil.commonBucket("etp_holder_e");
				bucket.upsert(doc);
				jsonObject = null;
				doc = null;				
			}
		...
		if(null!=holderDetList && holderDetList.size()>0)
		{
			for(HolderDetailVO detVO:holderDetList)
			{
				...
				Bucket bucket = ClusterUtil.commonBucket("etp_holder_det_e");
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public static void insertBaseInfo(JSONObject baseObj,String companyId,JSONObject obj) throws Exception
	{
		//企业对象
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public static void holder(JSONArray tuziArray,String bucketName,String companyId)
	{
		if(null != tuziArray && tuziArray.size()>0)
		{
			HolderVO holderVO = null;
		...
		if(!changArray.isEmpty())
		{
			...
			{				
				...
				if(null != changeVO && null != changeVO.getChangeEvent())
				{
					...
					Bucket bucket = ClusterUtil.commonBucket("etp_event_item_e");
					bucket.upsert(doc);
					bucket=null;
				}
				remarkVO = null;
