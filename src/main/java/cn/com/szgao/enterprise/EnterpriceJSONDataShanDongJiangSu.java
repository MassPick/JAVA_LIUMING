 ...
 *
 */
public class EnterpriceJSONDataShanDongJiangSu{
	/**
	 * json与对象互转
	 */
	 ...
	 * 工具类，通过住所、登记机关获得行政区
	 */
	static AdministrationUtils u = new AdministrationUtils();
	//集群对象
	static Cluster cluster = CouchbaseCluster.create("192.168.0.251");
	/**
	 * 经营异常信息Bucket=etp_abnormal_e
	 */
	//static Bucket bucket_abnormal = cluster.openBucket("etp_abnormal_e","2015sz");	
	/**
	 * 分支机构Bucket=etp_branch_e
	 */
	//static Bucket bucket_branch = cluster.openBucket("etp_branch_e","2015sz");
	/**
	 * 抽查检查信息Bucket=etp_check_e
	 */
	//static Bucket bucket_check = cluster.openBucket("etp_check_e","2015sz");
	/**
	 ...
	 */
	//static Bucket bucket_illel = cluster.openBucket("etp_illelnfo_e","2015sz");
	/**
	 ...
	 */
	//static Bucket bucket_mort = cluster.openBucket("etp_mortgage_e","2015sz");
	/**
	 * 行政处罚信息Bucket=etp_punish_e
	 */
	//static Bucket bucket_punish = cluster.openBucket("etp_punish_e","2015sz");
	/**
	 * 股权出质登记信息Bucket=etp_remise_e
	 */
	//static Bucket bucket_remise = cluster.openBucket("etp_remise_e","2015sz");
	/**
	 * 清算信息Bucket=etp_settlement_e
	 */
	//static Bucket bucket_settlement = cluster.openBucket("etp_settlement_e","2015sz");
	/**
	 * 公司基本信息Bucket=etp_settlement_e
	 */
	static Bucket bucket_etp = cluster.openBucket("etp_t","2015sz");	
	//日志对象
	private static Logger log = LogManager.getLogger(EnterpriceJSONDataShanDongJiangSu.class);
	//根据字符串生成UUID对象
	static NameBasedGenerator nbg = Generators.nameBasedGenerator(NameBasedGenerator.NAMESPACE_DNS);
	//变更前修改内容
	static List<String> beforeList = new ArrayList<String>();
	//变更后修改内容
	static List<String> afterList = new ArrayList<String>();
	//公司ID集合
	static List<String> companyIdList = new ArrayList<String>(); 
	static int count = 0;
	...
	private static void readFileByLines(File file) throws IOException, ParseException{		
		...
		try
		{			
			JSONObject baseObj = jsobj.getJSONArray("基本信息").getJSONObject(0);
			if(!baseObj.containsKey("注册号"))
			{
				log.info(file.getPath()+"...."+"无注册号");
				return;
			}
			String num = baseObj.get("注册号").toString();
			if(0 != num.hashCode())
			{
		...
		try{
			jsonDoc = bucket_etp.get(companyId, 5000, TimeUnit.MILLISECONDS);
		}
		catch(Exception e)
		{
			log.info(e);
	...
	*/	
			...
			{
				//JSONArray changArray,String companyId,JSONObject datas
				if(null != companyId)
				{
					insertChange(jsobj.getJSONArray(key),companyId);
				}
			}
			else if("经营范围信息".equals(key))
			{
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public static void insertBaseInfo(JSONObject baseObj,String companyId,JSONObject obj) throws ParseException
	{
		//企业对象
		...
		commonInsert(bucket_etp,doc);
		//bucket_etp.upsert(doc,5000,TimeUnit.MILLISECONDS);
		jsonObject = null;
		doc = null;
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public static void holder(JSONArray tuziArray,String bucketName,String companyId) throws ParseException
	{
		if(null != tuziArray && tuziArray.size()>0)
		{
			...
			for(int i = 0; i < tuziArray.size();i++)
			{
				...
				//股东
				holderVO.setHolder(tempVO.getHolder());
				//股东类型
				holderVO.setType(tempVO.getType());
				//证件类型
				holderVO.setLicenseType(tempVO.getLicenseType());
				//证件号码
				holderVO.setLicenseNum(tempVO.getLicenseNum());
				//出资方式
				holderVO.setEquityPart(tempVO.getEquityPart());
				holderVO.setRemark(remarkVO);				
				//把股东对象转换成JSON对象
				jsonObject = JsonObject.fromJson(gs.toJson(holderVO));
				holderVO = null;
				doc = JsonDocument.create(holderId, jsonObject);
				commonInsert(bucket_holder, doc);
				...
				//股东姓名
				holderDetVO.setHolder(tempVO.getHolder());
				//股东类型
				holderDetVO.setType(tempVO.getType());
				//认缴出资额	        别名：认缴出资额(万元)
				holderDetVO.setSubcriCapital(tempVO.getSubcriCapital());
				// 认缴出资方式	别名：出资方式、认缴明细
				holderDetVO.setConMethod(tempVO.getConMethod());
				//认缴日期
				holderDetVO.setConsidDate(tempVO.getConsidDate());
				//实缴金额
				holderDetVO.setActualCapital(tempVO.getActualCapital());
				//实缴方式
				holderDetVO.setFactMethod(tempVO.getFactMethod());
				//实缴日期
				holderDetVO.setActualDate(tempVO.getActualDate());
				//认缴额
				holderDetVO.setConCapital(tempVO.getConCapital());
				//实缴额
				holderDetVO.setFactCapital(tempVO.getFactCapital());
				remarkVO.setBucketName(remarkVO.getHolder()+"详细信息");
				if(null != holderDetVO)
				{
			...
			try {
				while(iter.hasNext())
				{
					...
					if(null != tempVO.getManagerName1() && null != tempVO.getPosition1())
					{
						...
						mainVO = null;
						doc = JsonDocument.create(UUID.randomUUID().toString(), jsonObject);
						commonInsert(bucket_mhold,doc);
						//bucket_mhold.upsert(doc,5000,TimeUnit.MILLISECONDS);
						jsonObject = null;
						doc = null;
					}
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public static void insertChange(JSONArray changArray,String companyId) throws ParseException
	{
		if(!changArray.isEmpty())
		{
			...
			{				
				...
				doc = JsonDocument.create(UUID.randomUUID().toString(), jsonObject);
				//写库
				commonInsert(bucket_change,doc);
				remarkVO = null;
				changeVO = null;
				jsonObject = null;
				doc = null;
	 ...
	 * @param doc 文档
	 */
	public static void commonInsert(Bucket bucket,JsonDocument doc)
	{
		try
		{
