...
/**
 *没有利用CB    是直接 先取json 再取html       通过法院名查询的裁判文书（旧版高院）
 * 
 * @author liuming
 * @Date 2016年6月16日 下午2:32:12
 */
public class ExtractionJsonAndHtmlToTxtOldJson {
	/**
	 * 写日志
	 */
	private static Logger logger = LogManager.getLogger(ExtractionJsonAndHtmlToTxtOldJson.class.getName());
	private static Logger log = LogManager.getLogger(ExtractionJsonAndHtmlToTxtOldJson.class.getName());
...
//		File file = new File("E:\\法院详细页面\\通过法院名查询的裁判文书（旧版高院）");
//
...
//		util.initData(); // 查询行政区
		...
		File file = new File(
				"F:\\法院详细页面\\通过法院名查询的裁判文书（旧版高院）\\西藏");
		
//		List<File> fileList=new AarryList<File> 
		util = new AdministrationUtils();
		util.initData(); // 查询行政区
...
//				"四川","天津","西藏","新疆","新疆建设兵团","云南","浙江","之前遗漏的","重庆" 
		};
		
		for (int i = 0; i < cArray.length; i++) {
			file = new File(
					"F:\\法院详细页面\\通过法院名查询的裁判文书（旧版高院）\\"+cArray[i]);
			Bucket bucket = null;
...
//		}
	...
	private static void show(File file, Bucket bucket) throws Exception {
		if (file.isFile()) {
			if ((file.getName().indexOf("resource") != -1 || file.getName().indexOf("url") != -1)
					&& "json".equals(file.getPath().substring(file.getPath().lastIndexOf(".") + 1))) {
				long da = System.currentTimeMillis();
				create(file, bucket);
		...
		for (File fi : files) {
			if (fi.isFile()) {
				...
				if ((fi .getPath().indexOf("resource") != -1 || fi.getPath().indexOf("url") != -1)
						&& "json".equals(fi.getPath().substring(fi.getPath().lastIndexOf(".") + 1))) {
					long da = System.currentTimeMillis();
	...
	private static <ObjectDataVO, JSONObject> void create(File file, Bucket bucket)
		...
		try {
			...
			while ((temp = reader.readLine()) != null) {
				...
				if (countNum % 10000 == 0) {
					countP++;
					String folderPathUn = "E:\\法院详细页面(清洗)_161025\\通过法院名查询的裁判文书（旧版高院）";
					String filePathUn = "E:\\法院详细页面(清洗)_161025\\通过法院名查询的裁判文书（旧版高院）\\"+"A1" + (countP)+"_"+DateUtils.getDateyyyyMMddhhmmss() + "通过法院名查询的裁判文书（旧版高院）.json";
					// 创建文件夹
				...
				// 由JSON路径得到HTML的路径
				// C:\\data\\新版裁判文书JSON+HTML\\2016年\\JSON\\JSON20160115-20160101\\2016-01-04\\刑事案件-2016-01-04.json
				// C:\\data\\新版裁判文书JSON+HTML\\2016年\\HTML\\HTML20160115-20160101

				// E:\法院详细页面\通过法院名查询的裁判文书（旧版高院）\新疆建设兵团\新疆建设兵团\\url\\阿克苏垦区人民法院.json

				if (filePath.indexOf("url") != -1) {
					fileHtmlpath = filePath.substring(0, filePath.indexOf("url"))
							+ file.getName().substring(0, file.getName().lastIndexOf(".")) + "\\"
							+ StringUtils.getUUID(archJson.getDetailLink().toString()) + ".html";
				}
				if (filePath.indexOf("resource") != -1) {
					fileHtmlpath = filePath.substring(0, filePath.indexOf("resource"))
							+ file.getName().substring(0, file.getName().lastIndexOf(".")) + "\\"
							+ StringUtils.getUUID(archJson.getDetailLink().toString()) + ".html";
				}
				...
				try {
					...
					log.error(" 找不到页面 :  " + e.getMessage());
					// 合并vo
					finalVo = heBinVo(archJson, archHtml, 4);
					if(finalVo!=null){
				...
				try {
					archHtml = ExtractionHtmlOld.getVoFromHtml(htmFile);
	 ...
	 * @return
	 */
	public static WholeCourtVO heBinVo(WholeCourtVO archJson, WholeCourtVO archHtml, Integer dataFrom) {
		
		if(archJson==null&&archHtml==null){
			return null;
		}
		String[] array = null;
...
//					public static String[] JUDGES = { "见习书记员","代书记员","代理审判员",  "代理审判长" , "人民陪审员","审判长", "审判员", "书记员", "执行长","执行员", };
				...
				// 数据来源 1 旧版高院 2 旧版地方 3 新版 4 旧版高院按法院查
