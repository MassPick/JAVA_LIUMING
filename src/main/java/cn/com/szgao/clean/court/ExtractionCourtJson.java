 ...
 */
public class ExtractionCourtJson {
	 ...
	 * 写日志
	 */
	private static Logger logger = LogManager.getLogger(ExtractionCourtJson.class.getName());
...
//		File file = new File("E:\\地方法院\\JSON的省份");  //232W
		
		
		//---------------------------------------数据来源    1   旧版高院    2  旧版地方法院   3  新版  4 旧版高院按法院名查 41      5  旧版地方法院Excel   
		File file = new File("E:\\地方法院\\EXCEL的省份\\通过法院名在裁判文书上采集的");    //总数量 10238568
//		File file = new File("E:\\地方法院\\EXCEL的省份\\通过法院名在裁判文书上采集的\\中国裁判文书网-法院名查询2（15省）\\中国裁判文书网-法院名查询2（15省）\\query_results\\海南");
	...
	private static <ObjectDataVO, JSONObject> void create(File file, Bucket bucket)
		...
		try {
			...
			while ((temp = reader.readLine()) != null) {
				...
				if (countNum % 10000 == 0) {
					countP++;
					String folderPathUn = "E:\\法院旧版本文书网Json";
					String filePathUn = "E:\\法院旧版本文书网Json\\" + (countP)+"_"+DateUtils.getDateyyyyMMddhhmmss() + ".json";
					// 创建文件夹
