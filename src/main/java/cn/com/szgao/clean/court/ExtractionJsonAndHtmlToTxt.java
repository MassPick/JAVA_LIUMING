 ...
 * 
 * @author liuming
 * @Date 2016年6月22日 下午3:55:23
 */
public class ExtractionJsonAndHtmlToTxt {
	/**
	 * 写日志
	 */
	private static Logger logger = LogManager.getLogger(ExtractionJsonAndHtmlToTxt.class.getName());
	private static Logger log = LogManager.getLogger(ExtractionJsonAndHtmlToTxt.class.getName());
		...
		try {
			...
			while ((temp = reader.readLine()) != null) {
				...
				try {
					...
					finalVo.setUpdateTime(DateUtils.getDateyyyyMMddhhmmssZZ());
					writerString(fwUn, StringUtils.GSON.toJson(finalVo));
					continue;
				}

				try {
					archHtml = ExtractionHtml.getVoFromHtml(htmFile);
