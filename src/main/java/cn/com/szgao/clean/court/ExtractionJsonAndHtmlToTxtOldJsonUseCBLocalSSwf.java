...
//import cn.com.szgao.enterprise.ExtractthepeopleText;
...
/**
 * 以swf为准，利用CB进行数据清洗    地方法院html  E:\法院详细页面\地方   swf   以湖北为主
 * 
 * @author liuming
 * @Date 2016年6月16日 下午2:32:12
 */
public class ExtractionJsonAndHtmlToTxtOldJsonUseCBLocalSSwf {
	/**
	 * 写日志
	 */
	private static Logger logger = LogManager.getLogger(ExtractionJsonAndHtmlToTxtOldJsonUseCBLocalSSwf.class.getName());
	private static Logger log = LogManager.getLogger(ExtractionJsonAndHtmlToTxtOldJsonUseCBLocalSSwf.class.getName());
	...
	public static void main(String[] args) throws Exception {
		...
		File file = new File("E:/法院详细页面/地方/swf");
		...
		try {
			...
			String[] cArray = new String[]{
					"安徽省","北京市","福建省","甘肃省","广东省","广西省","贵州省","海南省",
					"河北省","河南省","黑龙江省",
//					"湖北省","湖南省","吉林省","江苏省","江西省",
//					"辽宁省",	"内蒙古", 	"青海省","山东省",
//					"山西省","陕西省","上海市",
//					"四川省","天津市", "新疆省", "云南省","浙江省","之前网站因网络异常后边补下的","重庆市" 
			};
			
//			for (int i = 0; i < cArray.length; i++) {
//				file = new File(
//						"E:/法院详细页面/地方/"+cArray[i]);
//				// bucket = connectionBucket(bucket);
//				try {
//					show(file, bucket);
//
...
//			while ((temp = reader.readLine()) != null) {
				...
				if (countNum % 10000 == 0) {
					countP++;
					String folderPathUn = "D:\\法院详细页面(清洗)\\地方\\swf";
					String filePathUn = "D:\\法院详细页面(清洗)\\地方\\swf\\" + (countP)+"_"+DateUtils.getDateyyyyMMddhhmmss()  + "地方swf.json";
				...
				try {
					...
					// 合并vo
					finalVo = heBinVo(archJson, archHtml, 86);
					if(finalVo!=null){
						finalVo.setFilePathHtml(fileHtmlpath);
						finalVo.setPath("swf");
						finalVo.setWholeCourtId(key);
						
//						if(StringUtils.isNull(finalVo.getTitle())){
//							// 此处用的法院名+文书类型+案号当标题：
//							String title = (finalVo.getCourtName() != null ? finalVo.getCourtName() + " " : "")
//									+ (finalVo.getWritType() != null ? finalVo.getWritType() + " " : "")
//									+ (finalVo.getCaseNum() != null ? finalVo.getCaseNum() : "");
//							finalVo.setTitle(title); // 标题 √
//						}
					...
					countNum++;
					System.out.println(countNum);
					log.error("解析html异常 archHtml = ExtractionJsonAndHtmlToTxtOldJsonUseCBLocalSO :  " + e.getMessage());
					e.printStackTrace();
//					// 合并vo
//					finalVo = heBinVo(archJson, archHtml, 86);
//					finalVo.setFilePathHtml(fileHtmlpath);
