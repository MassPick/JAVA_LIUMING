...
/**
 * 将旧版本的高院文书 json    写到  json       
 * 
 *     E:\地方法院\EXCEL的省份\通过法院名在裁判文书上采集的     铭
 *     
 *     F:\法院详细页面\通过法院名查询的裁判文书（旧版高院） 小慧
 * 
 *  通过法院名查询的裁判文书（旧版高院）  json    写到  json    
 *  
 *  
 * 
 * @author liuming
 ...
 */
public class ExtractionCourtJson2 {
	 ...
	 * 写日志
	 */
	private static Logger logger = LogManager.getLogger(ExtractionCourtJson2.class.getName());
...
//		File file = new File("E:\\地方法院\\JSON的省份");
		...
		//---------------------------------------数据来源    1   旧版高院    2  旧版地方法院   3  新版  4 旧版高院按法院名查   41 通过法院名在裁判文书上采集的      5  旧版地方法院Excel   
...
//			bucket.close();
	...
	private static void show(File file, Bucket bucket) throws Exception {
		if (file.isFile()) {
			if ("json".equals(file.getName().substring(file.getName().lastIndexOf(".") + 1))  && ( file.getName().indexOf("resource")!=-1  ||  file.getName().indexOf("url")!=-1 )  ) {
...
//				if ("json".equals(fi.getName().substring(fi.getName().lastIndexOf(".") + 1))) {
			...
			} else if (fi.isDirectory()&& ( file.getName().indexOf("resource")!=-1  ||  file.getName().indexOf("url")!=-1 ) ) {
				show(fi, bucket);
			} else {
				continue;
			}
	...
	private static <ObjectDataVO, JSONObject> void create(File file, Bucket bucket)
		...
		try {
			...
			while ((temp = reader.readLine()) != null) {
				...
				if (countNum % 10000 == 0) {
					countP++;
					String folderPathUn = "E:\\通过法院名查询的裁判文书（旧版高院）Json";
					String filePathUn = "E:\\通过法院名查询的裁判文书（旧版高院）Json\\" + (countP)+"_"+DateUtils.getDateyyyyMMddhhmmss() + ".json";
					// 创建文件夹
