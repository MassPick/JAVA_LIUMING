...
/**
 * 裁判文书提取HTML写入couchbase库
 * 
 * @author Administrator
 *
 */
public class Get2suitTypeUtils {
	...
	public static String[] CLIENT_END = { "一案", "纠纷", "争议", "认定", "确认", "认为", "辩称", "承担", "负担", "签订", "无异议", "涉及", "诉称",
			"改判", "作为", "同居", "同居生活", "副本", "享受", "相互认识", "婚姻关系", "驳回", "赔偿", "通告", "纠纷", "未履行生效法律文书", "未履行法律文书",
			"申请强制执行", "受理", "生效" };

	public static String[] CLIENT_END_XinShi = { "一案", "纠纷", "争议", "认定", "确认", "认为", "辩称", "承担", "负担", "签订", "无异议",
			"涉及", "诉称", "改判", "作为", "同居", "同居生活", "副本", "享受", "相互认识", "婚姻关系", "驳回", "赔偿", "通告",
	...
	// 被告
	public static String[] DEFENDANT = { "被上诉人", "被执行人", "被申诉人", "被申请人", "被申请执行人", "原审被告人", "原审原告", "罪犯", "被告",
			"赔偿义务机关", "一审被告", "二审被上诉人" };

	// 原告
	public static String[] PLAINTIFF = { "第三人", "诉讼代理人", "辩护人", "上诉人", "申诉人", "申请执行人", "申请人", "执行人", "原审被告", "赔偿请求人",
			"原公诉机关", "公诉机关", "执行机构", "原告", "复议机关", "申请复议人", "一审原告", "委托代理人", "法定代表人", "起诉人", "移送执行机构", "二审上诉人", "原审第三人",
	...
	public static String[] KEYWORDKE = { "申请再审人", "被上诉人", "二审被上诉人", "原审被告人", "原审第三人", "二审上诉人", "一审被告", "一审原告", "被申请人",
			...
			"公诉机关", "复议机关", "委托代理人", "委托代理", "特别授权代理", "四被上诉人委托代理人", "两上诉人的委托代理人", "移送执行机构", "诉讼代理人", "法定代表人", "申请复议人",
			"被上诉人", "被申诉人", "被执行人", "反诉被告", "反诉原告", "原审被告", "原审原告", "执行人", "负责人", "上诉人", "起诉人", "申诉人", "被告人", "原告人",
			"被告", "原告", "罪犯", "第三人" };
...
//		}
		...
		WholeCourtVO wc=getCaseNumFormContent(doc);
		String caseNum1=wc.getCaseNum1();
		String caseBrief=wc.getCaseBrief();
		arch.setCaseNum1(caseNum1);
		arch.setCaseBrief(caseBrief); 
		
		// arch_info = getCourtInfo(doc);
		// arch = arch_info;
		...
		// "//div[@id='contentTitle']",1);
//		variable = HtmlUtils.getContentFromXpath_SX(doc,
//				"TEXT-ALIGN: center; LINE-HEIGHT: 25pt; MARGIN: 0.5pt 0cm; FONT-FAMILY: 黑体; FONT-SIZE: 18pt;", 1, 1);

		
//		*[@id="DivContent"]/div[15]
		//<div style="LINE-HEIGHT: 25pt;TEXT-ALIGN:justify;TEXT-JUSTIFY:inter-ideograph; TEXT-INDENT: 30pt; MARGIN: 0.5pt 0cm;FONT-FAMILY: 仿宋; FONT-SIZE: 16pt;">原告五矿二十三冶建设集团第一工程有限公司与被告郑湘、陈湘华、唐友江、李君建设工程合同纠纷一案，本院于2013年8月16日立案受理后，依法组成合议庭进行审理，原告于2013年8月22日向本院提出财产保全的申请，要求冻结被告郑湘、陈湘华、唐友江、李君名下12900000元银行存款或查封同等价值的财产，并提供了担保。本院于2013年8月22日依法作出了（2013）潭中民二初字第37-1号民事裁定书，裁定冻结被告郑湘、陈湘华、唐友江、李君名下12900000元银行存款或查封、扣押其同等价值的其他财产。本院于2013年9月14日向被告唐友江、李君公告送达了起诉状副本、应诉手续及开庭传票、（2013）潭中民二初字第37-1号民事裁定书。本院于2013年12月17日、2014年8月25日对本案公开开庭进行了审理。2013年12月17日，原告的委托代理人邹命、被告郑湘及其委托代理人赵铮、被告陈湘华到庭参加诉讼，被告唐友江、李君经本院依法传唤，无正当理由未到庭。2014年8月25日，原告的委托代理人邹命、被告郑湘及其委托代理人赵铮、被告陈湘华、被告唐友江、李君及其委托代理人黄华林均到庭参加诉讼。本案现已审理终结。</div>
		// *[@id="border_in"]/div/ul/li[1]

		// variable=getCourTittleOld(doc, 1);
//		arch.setTitle(variable); // 标题 √

		// *[@id="Content"]/div[2]/div/table/tbody/tr/td

		// 提取发布时间
		// variable = getCourPublishDateOld(doc, 1);
//		variable = HtmlUtils.getContentFromXpath(doc, "//div[@id='Content']/div[2]/div/table/tbody/tr/td");
...
//		writType = HtmlUtils.getContentFromXpath(doc, "//div[@id='DivContent']/div[2]", 1);// 文书类型
//
...
//		variable = HtmlUtils.getContentFromXpath(doc, "//div[@id='DivContent']/div[1]", 1);
//		if (variable != null && !"".equals(variable)) {
//			arch.setCourtName(variable); // 法院 √
//		}
...
//		}
		...
		variable = HtmlUtils.getContentFromXpath(doc, "//div[@id='DivContent']/div[3]", 1);// 案号
		// √
...
//		// 审判员等
//		WholeCourtVO vo = getJudgeNew(doc, 1);
...
//		arch.setLaws(laws);
		...
		System.out.println("---------------------------------------");
		System.out.println("一审的案号" + arch.getCaseNum1());
...
//		arch.setWholeCourtId(file.getName().substring(0, file.getName().lastIndexOf(".")));
	 ...
	 * @return String  
	 * @author liuming
	 * @date 2016年11月17日  下午4:43:23
	 */
	public static WholeCourtVO getCaseNumFormContent(Document doc ){
		String reslut = null;
		if (doc == null) {
			return null;
		}
...
//						fa++;
						...
						if(temp1.indexOf("一案")!=-1){
							
							vo.setCaseBrief(temp1);
							reslut=getCaseNum(temp1);
							vo.setCaseNum1(reslut);
							fa=1;
							return vo;
//							break;
						}
	 ...
	 */
	public static WholeCourtVO getclient(Document doc, int flag) {
	 ...
	 * 
	 * if (StringUtils.isNull(text)) { return null; } if (text.length() > 200) {
	 * System.out.println(text.substring(0, 199)); } for (String val :
	 * CourtData.LISTCasecause_small_small) { if (text.indexOf(val) != -1) {
	 * return val; } } for (String val : CourtData.LISTCasecause_small) { if
	 * (text.indexOf(val) != -1) { return val; } } for (String val :
	 * CourtData.LISTCasecause_middle) { if (text.indexOf(val) != -1) { return
	 * val; } } for (String val : CourtData.LISTCasecause_big) { if
	 * (text.indexOf(val) != -1) { return val; } }
	 * 
	 * String result = null;
	 * 
	 * return result; }
	 ...
	 */
	public static WholeCourtVO getJudgeNew(Document doc, int flag) {
		...
		String xpath = "//*[@id='DivContent']/div[position()>3]";
		...
		try {
			...
			for (Object o : rs) {
				if (o instanceof Element) {
					...
					Attributes as = ((Element) o).attributes();
					for (Attribute attribute : as) {
						if ("style".equals(attribute.getKey()) && !StringUtils.isNull(attribute.getValue())
								&& attribute.getValue().toLowerCase().indexOf("right") != -1) {
	...
	public static WholeCourtVO getCourtInfo(Document doc) {

		WholeCourtVO arch_info = new WholeCourtVO();
		String reslut = null;
		if (doc == null) {
			return null;
		}

		arch_info.setTitle(doc.title());
		TreeSet<String> ts = new TreeSet<String>();
		...
		for (Element element_all : all) {
			...
			if ("a".equals(tagName)) {
				...
				for (Attribute attribute : as) {
					...
					if ("name".equals(attribute.getKey()) && "WBSB".equals(attribute.getValue())) {
						System.out.println("当事人标签");
						str_temp = "";
						flag_i = 1;
					} else if ("name".equals(attribute.getKey()) && "DSRXX".equals(attribute.getValue())) {
						System.out.println("当事人标签标签开始");
						flag_i = 1;
						str_temp = "";
					} else if ("name".equals(attribute.getKey()) && "SSJL".equals(attribute.getValue())) {

						arch_info.setInfo_clinets(str_temp);
						System.out.println("审理经过标签开始");
						flag_i = 1;
						str_temp = "";
					} else if ("name".equals(attribute.getKey()) && "AJJBQK".equals(attribute.getValue())) {
						arch_info.setInfo_trial_by(str_temp);
						System.out.println("诉称标签开始");
						flag_i = 1;
						str_temp = "";
					} else if ("name".equals(attribute.getKey()) && "CPYZ".equals(attribute.getValue())) {
						arch_info.setInfo_claims_plai3(str_temp);
						System.out.println("本院认为标签开始");
						flag_i = 1;
						str_temp = "";
					} else if ("name".equals(attribute.getKey()) && "PJJG".equals(attribute.getValue())) {

						arch_info.setInfo_court_held3(str_temp);
						System.out.println("裁判结果标签开始");
						flag_i = 1;
						str_temp = "";
					} else if ("name".equals(attribute.getKey()) && "WBWB".equals(attribute.getValue())) {
						arch_info.setInfo_approval(str_temp);
						System.out.println("审判人员标签开始");
						flag_i = 1;
						str_temp = "";
					}
			...
			System.out.println(element_all.data());
			System.out.println("3---------------");
			System.out.println(element_all.id());
			System.out.println("---------------");
			Attributes as = element_all.attributes();
			for (Attribute attribute : as) {
				// System.out.println(attribute.getKey());
				...
				// <div style="LINE-HEIGHT: 25pt; TEXT-INDENT: 30pt; MARGIN:
				// 0.5pt 0cm;FONT-FAMILY: 宋体; FONT-SIZE: 15pt;">
				// 上诉人黄道益活络油有限公司（简称黄道益公司）因商标异议复审行政纠纷一案，不服北京市第一中级人民法院（简称北京市第一中级人民法院）（2014）一中知行初字第489号行政判决，向本院提起上诉。本院2014年9月11日受理本案后，依法组成合议庭进行了审理。本案现已审理终结。
				// </div>
				...
				// <div style="TEXT-ALIGN: right; LINE-HEIGHT: 25pt; MARGIN:
				// 0.5pt 36pt 0.5pt 0cm;FONT-FAMILY: 宋体; FONT-SIZE: 15pt;">
				// 审 判 长 刘 辉
				// </div>

				else if ("style".equals(attribute.getKey()) &&

				("TEXT-ALIGN: right; LINE-HEIGHT: 25pt; MARGIN: 0.5pt 72pt 0.5pt 0cm;FONT-FAMILY: 仿宋; FONT-SIZE: 16pt;"
						.equals(attribute.getValue())
						|| "TEXT-ALIGN: right; LINE-HEIGHT: 25pt; MARGIN: 0.5pt 36pt 0.5pt 0cm;FONT-FAMILY: 宋体; FONT-SIZE: 15pt;"
								.equals(attribute.getValue()))) {
					// System.out.println(element_all.text());
					// fa++;
					String temp1 = element_all.text();
					temp1 = temp1.replaceAll("[\n,\t,\r,\\s,&nbsp;]", "");
					str_temp_judge += temp1 + "@_@";
					System.out.println(temp1);
				}
		...
		arch_info.setJudges(str_temp_judge);
		return arch_info;
	}

	public static WholeCourtVO getCourtInfo_Txt_1(WholeCourtVO arch, Document doc) {
		...
		for (Element element_all : all) {
			...
			for (Attribute attribute : as) {
				...
				("LINE-HEIGHT: 25pt;TEXT-ALIGN:justify;TEXT-JUSTIFY:inter-ideograph; TEXT-INDENT: 30pt; MARGIN: 0.5pt 0cm;FONT-FAMILY: 仿宋; FONT-SIZE: 16pt;"
					...
					String temp_t = "";
					if (temp1.length() > 31) {
						String s_t[] = temp1.split("[：。:]");
						temp_t = s_t[0];
						// temp_t=temp1.substring(0,30);
					} else {
						temp_t = temp1;
					}

					if (temp_t.indexOf("一案") != -1) {
						...
						if (flag_i == 10) {
							...
							//
							arch_info.setInfo_claims_plai1(str_temp);
							System.out.println("辩称标签开始");
						}
					...
					else if (temp_t.indexOf("经") != -1 && temp_t.indexOf("查明") != -1) {
						if (flag_i == 10) {
							// 当事人
							arch_info.setInfo_clinets(str_temp);
						} else if (flag_i == 20) {
							...
							// 原告诉称
							arch_info.setInfo_argued_defend1(str_temp);
							System.out.println("辩称标签开始");
						} else if (flag_i == 40) {
							// 被告辩称
							arch_info.setInfo_argued_defend1(str_temp);
						...
						if (flag_i == 10) {
							...
							// 本院查明
							arch_info.setInfo_court_find1(str_temp);
						}

						// 本院认为
						flag_i = 60;
						if (isApproval(temp1) == 1) {// 如果判决如下：在本院认为同一段落
							arch_info.setInfo_court_held1(temp1);
							flag_i = 200;
							str_temp = "";
						}

						System.out.println("本院认为");
	 ...
	 * @return Integer
	 * @author liuming
	 * @date 2016年9月23日 上午10:21:22
	 */
	public static Integer isApproval(String temp1) {
		int re = 0;

		if (temp1.length() > 8) {
			for (String key : THEVERDICT) {
				if (temp1.substring(temp1.length() - 7).indexOf(key + "：") != -1) {
					re = 1;
					break;
				}
			}
	 ...
	 */
	/*
	 * public static String getLaws(String s1) { HashMap<String, String> hashLaw
	 * = new HashMap<String, String>();
	 * 
	 * String s2[] = s1.split("[，。]"); for (int i = 0; i < s2.length; i++) {
	 * String st = s2[i]; if (st.indexOf("》第") != -1 && st.indexOf("条") != -1 &&
	 * st.indexOf("《") != -1) {
	 * 
	 * if (st.indexOf("、") != -1) { String s21[] = st.split("、"); String tm_law
	 * = ""; for (String st2 : s21) { if (st2.indexOf("》第") != -1 &&
	 * st2.indexOf("条") != -1 && st2.indexOf("《") != -1) { tm_law =
	 * st2.substring(st2.indexOf("《"), st2.indexOf("》") + 1); String temp =
	 * st2.substring(st2.indexOf("《"), st2.indexOf("条") + 1);
	 * System.out.println(temp); hashLaw.put(temp, temp); } else if
	 * (st2.indexOf("第") != -1 && st2.indexOf("条") != -1) { String temp = tm_law
	 * + st2.substring(st2.indexOf("第"), st2.indexOf("条") + 1);
	 * System.out.println(temp); hashLaw.put(temp, temp); } }
	 * 
	 * } else { String temp = st.substring(st.indexOf("《"), st.indexOf("条") +
	 * 1); System.out.println(temp); hashLaw.put(temp, temp); } }
	 ...
	 * "key键---值: " + list.get(i) + "," + // hashLaw.get(list.get(i)));
	 * rels.append(list.get(i) + "</br>"); } if (rels.length() > 0) { rs =
	 * rels.toString(); rs = rs.substring(0,
	 * rs.toString().lastIndexOf("</br>")); } return rs; }
	 */
