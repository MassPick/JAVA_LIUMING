 ...
 * 只解析html
 * @author liuming
 * @Date 2016年6月22日 下午4:41:05
 */
public class ExtractionJsonAndHtmlToTxtOld {
	/**
	 * 写日志
	 */
	private static Logger logger = LogManager.getLogger(ExtractionJsonAndHtmlToTxtOld.class.getName());
	private static Logger log = LogManager.getLogger(ExtractionJsonAndHtmlToTxtOld.class.getName());
...
//				"E:/法院详细页面/中国裁判文书网展示文书详细页面（旧版高院）/zgcpwsw201503/cpwsw/ms");
		...
		File file = new File(
				"E:/法院详细页面/通过法院名查询的裁判文书（旧版高院）/新疆建设兵团/新疆建设兵团/三坪垦区人民法院/4a926e5e-b65d-5c95-aff6-1a65f4dacc1a.html");
	...
	private static <ObjectDataVO, JSONObject> void create(File file, Bucket bucket)
		...
		try {
			...
			if (countNum % 10000 == 0) {
				...
				String filePathUn = "D:\\lm\\log\\2016法院清洗后数据7\\" + (countP) + ".json";
				// 创建文件夹
