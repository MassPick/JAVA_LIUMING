...
/**
 * 算 html,json数量   json-html
 * @author MassPick
 * @Date 2016年11月10日 下午2:45:56
 */
public class CountHtmlAndJson {
	/**
	 * 写日志
	 */
	private static Logger logger = LogManager.getLogger(CountHtmlAndJson.class.getName());
	private static Logger log = LogManager.getLogger(CountHtmlAndJson.class.getName());
...
//		File file = new File("C:\\data\\文书新版本\\JSON\\JSON_cprq20150131-20150101\\2015-01-01\\民事案件-2015-01-01.json");
		...
		File file = new File("D:\\新版裁判文书网展示文书(以裁判日期查询)\\2015年\\JSON");
		
//		File file = new File("D:\\新版裁判文书网展示文书(以裁判日期查询)\\2013年\\JSON");
	...
	private static <ObjectDataVO, JSONObject> void create(File file, Bucket bucket)
		...
		try {
			...
			while ((temp = reader.readLine()) != null) {
				num_json++;
//				System.out.println("json 行："+num_json);
			}
			...
			int num_html=0;
			File file11 = new File(fileHtmlpath);
			if(!file11.exists()){
...
//				for(File f:files){
//					num_html++;
//					System.out.println("html数: "+num_html);
//				}
			}
			
			System.out.println(file.getPath()   +"#"+ num_json+"#"+num_html+"#"+(num_json-num_html));
