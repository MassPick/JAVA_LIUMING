...
public class NoticeCBToJson {
	static Logger logger = LogManager.getLogger(NoticeCBToJson.class);
	static long count = 0;
	static int i = 0;
/**
 * 公告CB写JSON
 ...
 */
	...
	public static void show() throws Exception {
		...
		while (true) {
			...
			logger.info("-----------" + count + "--------每次开始搜索下一批数据的KEY：" + key);
			if (null == key) {
				urlVariables.put("skips", 0);
				result = template.getForObject(
						"http://192.168.1.4:8092/courtPub/_design/courtPub/_view/courtPub?"
			...
			for (Rows r : jsn.getRows()) {
				i++;
				key = r.getKey();
//				System.out.println(DateUtils.toYMDOfChaStr_ESZZ2(time));
